...
describe("no-direct-query-hooks", () => {
	it("flags useQuery in a component file", () => {
		...
		expect(result[0].message).toContain("QO__");
	});
	...
	it("flags useQueryClient in a component file", () => {
		expect(lint(`const client = useQueryClient();`)).toHaveLength(1);
	});
	...
	it("does NOT flag useSuspenseQuery in a route file", () => {
		const result = lint(
			`const { data } = useSuspenseQuery(QO__todosList(search));`,
			"apps/web/src/routes/_authed/todos.tsx",
		);
		expect(result).toHaveLength(0);
	});

	it("allows useQuery in a colocated query file", () => {
		const result = lint(
			`const { data } = useQuery({ queryKey: ['todos'], queryFn: fetchTodos });`,
	...
	it("allows useMutation in a colocated query file", () => {
		const result = lint(
			`const mutation = useMutation({ mutationFn: createTodo });`,
			"src/features/todos/todos.queries.ts",
		);
