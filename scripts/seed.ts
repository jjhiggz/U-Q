...
const SAMPLE_NOTES = [
	...
	null,
	null,
	null, // Include nulls for variety
];
...
const generateNameInChat = (): string | null => {
	...
	if (style < 0.3) {
		...
		// Prefix + word style
		const prefix =
			CHAT_NAME_PREFIXES[Math.floor(Math.random() * CHAT_NAME_PREFIXES.length)];
		const word = faker.word.adjective();
		const suffix =
			CHAT_NAME_SUFFIXES[Math.floor(Math.random() * CHAT_NAME_SUFFIXES.length)];
...
const generateYoutubeUrl = (_artist: string): string | null => {
...
const generateInstagramUrl = (artist: string): string | null => {
	...
	const slug = toSlug(artist);
	const suffix =
		Math.random() > 0.7 ? `_music` : Math.random() > 0.5 ? `official` : "";
...
async function seed() {
	...
	console.log(
		`   🍌 Bananas: ${withBananas} songs (${totalBananas} total stickers)\n`,
	);
	...
	for (let i = 0; i < allSongs.length; i += batchSize) {
		...
		console.log(
			`✓ Inserted songs ${i + 1} - ${Math.min(i + batchSize, allSongs.length)}`,
		);
	}
