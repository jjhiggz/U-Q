								...
								<div className="flex items-start justify-between gap-4">
									...
									<div className="flex flex-col gap-2">
										...
										<Button
											...
											onClick={() => {
												if (
													confirm(`Remove "${userSong.title}" from the queue?`)
												) {
						...
						<div className="space-y-2">
							{(() => {
								...
								const allPointsTotal = songs.reduce(
									(sum: number, s: { points: number }) => sum + (s.points || 1),
									0,
								);
									...
									? songs.filter(
											(s: {
												...
												switch (searchCriteria) {
													...
													case "chatName":
														return (
															s.nameInChat?.toLowerCase().includes(query) ??
															false
														);
													case "genre":
														return (
								...
								return filteredSongs.map(
									(song: {
										...
										if (song.bananaStickers > 0 && hasBanana) {
											...
											const chanceInPoints =
												allPointsTotal > 0 ? songPoints / allPointsTotal : 0;
													...
													const headerContent = (
														<>
															...
															{song.bananaStickers > 0 && (
																...
																>
																	{Array.from({
																		length: Math.min(song.bananaStickers, 3),
																	}).map((_, i) => (
																		<img
																	...
																	{customPointsSongId === song.id ? (
																		<div className="flex items-center gap-1">
																			<Input
																				...
																				onChange={(e) =>
																					setCustomPointsValue(e.target.value)
																				}
																				onClick={(e) => e.stopPropagation()}
																			...
																			<Button
																				...
																				disabled={
																					!customPointsValue ||
																					parseInt(customPointsValue, 10) <= 0
																				}
																			>
																				Add
																			</Button>
