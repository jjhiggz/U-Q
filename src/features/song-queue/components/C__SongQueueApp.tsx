...
import { C__PinnedSongSection } from "./C__PinnedSongSection";
import { C__SpinWheelModal } from "./C__SpinWheelModal";
...
import { C__WheelPreviewModal } from "./C__WheelPreviewModal";
import { authClient } from "@/lib/auth-client";
import {
	useAdjustBananaStickers,
	useAdjustSongPoints,
	useArchivedSongs,
	useArchiveSong,
	useClearSongs,
...
export function C__SongQueueApp() {
	...
	const [customPointsSongId, setCustomPointsSongId] = useState<number | null>(
		null,
	);
	...
	const { data: archivedSongs = [] } = useArchivedSongs();
	...
	const userSong = useMemo(
		...
		() => {
			...
			return songs.find((song) => song.submittedByUserId === user.id) ?? null;
		},
		[songs, user?.id],
	);
	...
	const archiveMutation = useArchiveSong();
	const addPointsMutation = useAdjustSongPoints();
	const bananaMutation = useAdjustBananaStickers();
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (title.trim() && artist.trim() && (isKnownAdminUser || !userSong)) {
			submitMutation.mutate(
				...
				{ onSuccess: resetSubmissionForm },
			);
		}
	};

	// Helper to get song link from legacy fields or new field
	const getSongLinkFromSong = (song: {
					...
					<CardContent className="flex gap-3">
						...
						<Button
							...
							onClick={
								...
								() => {
									if (
										confirm("Are you sure you want to clear the entire queue?")
									) {
			...
			{userSong && !isKnownAdminUser && (
				<Card className="mb-6 border-green-500/50 bg-green-500/5">
					<CardHeader>
						...
						<CardDescription>
							You can edit your submission, or remove it to submit a different
							song
						</CardDescription>
					</CardHeader>
					<CardContent>
						{match(editingSongId === userSong.id)
							.with(true, () => (
								// Edit form
								<div className="space-y-4 p-4 bg-green-500/10 rounded-lg border border-green-500/20">
									...
									<div className="space-y-2">
										...
										<p className="text-xs text-muted-foreground">
											No Apple Music or other paid streaming services
										</p>
									</div>
									...
									<div className="border rounded-lg overflow-hidden">
										<button
											type="button"
											onClick={() =>
												setEditSocialLinksOpen(!editSocialLinksOpen)
											}
												...
												<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
													...
													<div className="flex items-center gap-2">
														...
														<Input
															...
															onChange={(e) =>
																setEditFacebookUrl(e.target.value)
															}
															className="flex-1"
														/>
													</div>
								...
								<div className="p-4 bg-green-500/10 rounded-lg border border-green-500/20">
									<div className="flex items-start justify-between gap-4">
										<div className="flex-1 min-w-0">
											...
											{(() => {
												const userSongLink =
													userSong.songLink || userSong.spotifyUrl;
												...
												return (
													<div className="flex flex-wrap items-center gap-1.5 mt-1.5">
														...
														{userSong.soundcloudUrl && (
															<a
																href={userSong.soundcloudUrl}
																target="_blank"
																rel="noopener noreferrer"
																className="w-5 h-5 rounded bg-orange-500 flex items-center justify-center hover:bg-orange-600 transition-colors"
																title="SoundCloud"
															>
														...
														{userSong.instagramUrl && (
															<a
																href={userSong.instagramUrl}
																target="_blank"
																rel="noopener noreferrer"
																className="w-5 h-5 rounded bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400 flex items-center justify-center hover:opacity-80 transition-opacity"
																title="Instagram"
															>
																<span className="text-white text-[8px] font-bold">
																	IG
														...
														{userSong.tiktokUrl && (
															<a
																href={userSong.tiktokUrl}
																target="_blank"
																rel="noopener noreferrer"
																className="w-5 h-5 rounded bg-black flex items-center justify-center hover:bg-gray-800 transition-colors"
																title="TikTok"
															>
																<span className="text-white text-[8px] font-bold">
																	TT
														...
														{userSong.facebookUrl && (
															<a
																href={userSong.facebookUrl}
																target="_blank"
																rel="noopener noreferrer"
																className="w-5 h-5 rounded bg-blue-600 flex items-center justify-center hover:bg-blue-700 transition-colors"
																title="Facebook"
															>
																<span className="text-white text-[8px] font-bold">
																	FB
											...
											<Button
												...
												onClick={
													...
													() => {
														if (
															confirm(
																`Remove "${userSong.title}" from the queue?`,
															)
														) {
															deleteMutation.mutate({ id: userSong.id });
														}
													}
												}
					...
					>
						<div className="flex items-center justify-between">
							...
							<Button variant="ghost" size="sm" className="h-8 w-8 p-0">
								{match(submitFormOpen)
									.with(true, () => <ChevronUp className="h-4 w-4" />)
									.with(false, () => <ChevronDown className="h-4 w-4" />)
									.exhaustive()}
							</Button>
						</div>
					</CardHeader>
								...
								<div className="border rounded-lg overflow-hidden">
									...
									{socialLinksOpen && (
										<div className="p-3 space-y-3 border-t">
											...
											<p className="text-xs text-muted-foreground">
												Help others follow the artist on social media
											</p>
										</div>
									)}
								</div>
					...
					<CardContent className="space-y-3 text-sm">
						<p className="text-muted-foreground">
							<span className="font-semibold text-foreground">
								The higher your points, the more likely you are to be picked
							</span>{" "}
						...
						<div className="space-y-2">
							...
							<ul className="list-disc list-inside space-y-1 text-muted-foreground">
								<li>
									<span className="text-foreground">Stay on the board</span> —
									Every time a song gets picked, all remaining songs get +1
									point
								</li>
								<li>
									<span className="text-foreground">
										Be a good person in chat
									</span>{" "}
									— I might give bonus points to cool people!
								</li>
							</ul>
						</div>
					</CardContent>
								...
								.with(false, () => (
									<div className="space-y-2">
										{(() => {
											...
											const bananaSongs = songs.filter(
												(s: { bananaStickers: number }) => s.bananaStickers > 0,
											);
											const totalBananaCount = bananaSongs.reduce(
												(sum: number, s: { bananaStickers: number }) =>
													sum + s.bananaStickers,
												0,
											);
											const allPointsTotal = songs.reduce(
												(sum: number, s: { points: number }) =>
													sum + (s.points || 1),
												0,
											);
											...
											const filteredSongs = match(query.length > 0)
												.with(true, () =>
													songs.filter(
														(s: {
															title: string;
															...
															return match(searchCriteria)
																...
																.with(
																	"genre",
																	() =>
																		s.genres?.toLowerCase().includes(query) ??
																		false,
																)
																.with(
																	"all",
											...
											return filteredSongs.map(
												...
												(song: {
													...
													const isOwnSong = song.submittedByUserId === user?.id;
													...
													const isExpanded =
														isOwnSong || expandedSongs.has(song.id);
													...
													if (song.bananaStickers > 0 && hasBanana) {
														...
														const chanceInBanana =
															totalBananaCount > 0
																? (song.bananaStickers / totalBananaCount) * 0.5
																: 0;
														const chanceInPoints =
															allPointsTotal > 0
																? (songPoints / allPointsTotal) * 0.5
																: 0;
														percentChance =
															(chanceInBanana + chanceInPoints) * 100;
													} else {
														// Non-banana songs only in points section
														const poolChance = hasBanana ? 0.5 : 1;
														const chanceInPoints =
															allPointsTotal > 0
																? songPoints / allPointsTotal
																: 0;
																...
																<div className="space-y-3">
																	...
																	<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
																		<Input
																			...
																			value={editNameInChat}
																			onChange={(e) =>
																				setEditNameInChat(e.target.value)
																			}
																		/>
																		<Input
																	...
																	<Input
																		...
																		value={editSongLink}
																		onChange={(e) =>
																			setEditSongLink(e.target.value)
																		}
																	/>
																	{/* Social Links - Collapsible */}
																				...
																				<div className="grid grid-cols-1 md:grid-cols-2 gap-2">
																					...
																					<div className="flex items-center gap-2">
																						...
																						<Input
																							...
																							value={editTiktokUrl}
																							onChange={(e) =>
																								setEditTiktokUrl(e.target.value)
																							}
																							className="flex-1 h-8 text-sm"
																						/>
																					</div>
													...
													if (!isExpanded) {
														const CollapsedWrapper = hasExtraContent
															? "button"
															: "div";
														return (
															<CollapsedWrapper
															...
															>
																{/* Rank */}
																<div
																	className={`w-6 text-center font-bold text-sm flex-shrink-0 ${isTop3 ? rankColors[rank - 1] : "text-muted-foreground"}`}
																>
																	{rank <= 3
																		? ["🥇", "🥈", "🥉"][rank - 1]
																		: `#${rank}`}
																...
																{song.bananaStickers > 0 && (
																	<span
																		...
																		title={`${song.bananaStickers} banana sticker${song.bananaStickers > 1 ? "s" : ""}`}
																	>
																		<img
																			src="/banana sticker.png"
																			alt="🍌"
																			className="w-4 h-4"
																		/>
																		{song.bananaStickers > 1 && (
													...
													return (
														<div
															key={song.id}
															className={`border rounded-lg transition-colors ${match(
																isOwnSong,
															)
																.with(
																	true,
																		...
																		{song.bananaStickers > 0 && (
																			...
																			>
																				{Array.from({
																					...
																					<img
																						key={`banana-exp-${song.id}-${i}`}
																						src="/banana sticker.png"
																						alt="🍌"
																						className="w-4 h-4"
																					/>
																				))}
																				{song.bananaStickers > 3 && (
																					<span className="text-xs text-amber-600">
																						+{song.bananaStickers - 3}
																					</span>
																				)}
																	...
																	<div className="flex items-center gap-2 flex-shrink-0">
																		...
																		{isKnownAdminUser && (
																			<div className="flex items-center gap-1">
																				{match(customPointsSongId === song.id)
																					.with(true, () => (
																						<div className="flex items-center gap-1">
																							<Input
																								type="number"
																						...
																						<>
																							<div className="flex flex-col gap-1">
																								<div className="flex items-center gap-1">
																									<Button
																										size="sm"
																										variant="ghost"
																										className="h-6 px-1.5 text-xs text-green-600 hover:text-green-700 hover:bg-green-100"
																										onClick={(e) => {
																											e.stopPropagation();
																											addPointsMutation.mutate({
																												id: song.id,
																								...
																								<div className="flex items-center gap-1">
																									<Button
																										size="sm"
																										variant="ghost"
																										className="h-6 px-1.5 text-xs text-red-600 hover:text-red-700 hover:bg-red-100"
																										onClick={(e) => {
																											e.stopPropagation();
																											addPointsMutation.mutate({
																												id: song.id,
																							...
																							<div className="flex items-center gap-0.5 border rounded px-1 py-0.5 bg-amber-50">
																								<Button
																									size="sm"
																									variant="ghost"
																									className="h-5 w-5 p-0 text-xs text-amber-700 hover:bg-amber-200"
																									onClick={(e) => {
																										e.stopPropagation();
																										bananaMutation.mutate({
																											id: song.id,
																											delta: -1,
																										});
																									}}
