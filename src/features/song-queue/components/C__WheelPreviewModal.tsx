...
export function C__WheelPreviewModal({
	...
	if (regularSectionDegrees > 0) {
		const regularPointsTotal = allSongs.reduce(
			(sum, s) => sum + (s.points || 1),
			0,
		);
	...
	if (song.bananaStickers > 0 && hasBanana) {
		...
		const allPointsTotal = allSongs.reduce(
			(sum, s) => sum + (s.points || 1),
			0,
		);
			...
			<DialogContent className="sm:max-w-md">
					...
					<DialogTitle className="flex items-center gap-2">
						{song.bananaStickers > 0 && (
							<span className="flex items-center gap-0.5">
								{Array.from({ length: Math.min(song.bananaStickers, 3) }).map(
									(_, i) => (
										<img
											key={`banana-${song.id}-${i}`}
								...
								{song.bananaStickers > 3 && (
									<span className="text-xs">+{song.bananaStickers - 3}</span>
								)}
							</span>
						)}
						{song.title}
				...
				<div className="flex flex-col items-center py-4">
					...
					<svg
						...
						aria-label="Preview of spin wheel"
					>
						...
						{song.bananaStickers > 0 && hasBanana && (
							<div className="mt-4 text-xs text-muted-foreground bg-muted rounded-lg p-3">
								<span className="font-medium text-amber-600">
									🍌 Banana sticker bonus! ({song.bananaStickers}x)
								</span>
								<br />
								Appears in both sections of the wheel, banana section weighted
								by sticker count
							</div>
						)}
